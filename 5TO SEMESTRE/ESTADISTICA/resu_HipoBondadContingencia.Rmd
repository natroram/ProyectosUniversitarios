---
title: "resumenHipotesisBondadContingencia"
...
est.prueba: z = (psomb - p.supuesta)/sqrt((p.supuesta*(1-p.supuesta))/n)
R.R
...
opinion <- read.table("clipboard", header=T)
...
```{r}
turnos<-c("matutino", "vespertino", "nocturno")
tipo<-c("defectuoso", "no defectuoso")
mat<-c(45,905)
vesp<-c(55,890)
noc<-c(70,870)

tabla.cont = cbind(mat,vesp,noc)
colnames(tabla.cont)<-turnos
rownames(tabla.cont)<-tipo
...
verificar valor p < 0.05 para rechazar Ho o p < nivel de significancia
